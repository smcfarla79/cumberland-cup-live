/**
 * One-off demo seed: complete Friday PM hole scores + finalize matches.
 * Back 9 (holes 10–18). Run: node scripts/seed-friday-pm-scores.mjs
 */
import fs from "fs";
...
/** Per-match hole winners for the back 9. */
const MATCH_SCRIPTS = {
  1: ["b", "a", "b", "a", "h", "b", "a", "b", "a"], // A 1 up (4-4 + last a? count: a=4 b=4 h=1 → halved)
  // Fix: want a clear finish with variety if more matches appear later
};

// Override match 1 to Team B 2 up for a clear board result
MATCH_SCRIPTS[1] = ["b", "a", "b", "h", "b", "a", "b", "h", "b"]; // B 4 up (5-2)
...
async function main() {
  ...
  const fri = (rounds ?? []).find((r) => /friday pm/i.test(r.name));
  if (!fri) throw new Error("Friday PM round not found");
  ...
  if (!matches?.length) {
    throw new Error("No Friday PM matches — set lineups first");
  }
  ...
  let players = [];
  {
    const withHcp = await sb.from("players").select("id,display_name,handicap");
    if (withHcp.error) {
  ...
  for (const match of matches) {
    ...
    const script =
      MATCH_SCRIPTS[match.match_number] ??
      ["a", "b", "h", "a", "b", "h", "a", "b", "h"];
